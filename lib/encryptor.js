...
// vigenereEncode(message: string, key: string): string -> shifts a message by another string
const vigenereEncode = (message, key) => message.split('').reduce((result, char, i) => result += shiftChar(char, abc.indexOf(key[i % key.length])), '');


// vignereDecode(message: string, key: string): string -> decoded string
const vignereDecode = (message, key) => message.split('').reduce((result, char, i) => result += shiftChar(char, -1 * abc.indexOf(key[i % key.length])), '');
