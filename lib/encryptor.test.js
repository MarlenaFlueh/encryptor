...
const {shiftChar, shiftByValue, rot13, vigenereEncode, vignereDecode} = require('../lib/encryptor');
...
describe('encryptor.js', () => {
  ...
  describe('vigenereEncode', () => {
    it('should encode a string by another string', () => {
      expect(vigenereEncode('hallo welt', 'baum')).to.be.equal('iafxp qqmt');
    });
  });

  describe('vigenereDecode', () => {
    it('should encode a string by another string', () => {
      expect(vignereDecode('iafxp qqmt', 'baum')).to.be.equal('hallo welt');
    });
  });
});
